<template>
  <div v-if="visible" class="fixed inset-0 bg-black bg-opacity-90 flex justify-center items-center">
    <div class="bg-card p-10 rounded-lg shadow-lg w-1/2 h-auto flex flex-col justify-between">
      ...
      <p class="text-foreground mt-4">
        La inscripción a este taller es completamente voluntaria, por lo que asume el compromiso de participar
        activamente
        en el mismo. En caso de reprobar este por asistencia, no podrá tomar ningún otro taller ofrecido por el
        Departamento de Informática en un plazo de al menos un año.
      </p>
      ...
      <div class="flex flex-col mt-6">
        ...
        <select class="bg-input border border-border rounded-lg mt-3 p-2 text-foreground" v-if="isConvalidated"
...
import type { WorkshopResponse } from "@/features/workshops/management/types/workshop_model";

import { useAuthStore } from "@/shared/stores/auth_store";

import { ref, onMounted, reactive } from "vue";
import type { CurriculumCourseResponse } from "@/features/academic/curriculum/types/curriculum_course_model";
...
import type { WorkshopsInscriptionsPost } from "@/features/workshops/inscription/types/workshop_inscription_model";
