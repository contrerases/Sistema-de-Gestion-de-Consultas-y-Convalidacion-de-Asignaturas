<template>
    <div class="w-screen h-[800px] max-w-screen-2xl mx-auto rounded-2xl flex flex-row border-2 border-border p-10">
      <!-- Sección de Login -->
      <div class="w-full lg:w-1/4 flex flex-col space-y-4">
        <div class="bg-card text-card-foreground rounded-[var(--radius)] shadow p-4">
        ...
        <!-- Sección de Logo -->
        <div class="bg-card  text-card-foreground rounded-[var(--radius)] shadow p-16 h-full flex justify-center items-center">
      ...
      <div class="w-full lg:w-3/4 bg-card text-card-foreground rounded-[var(--radius)] shadow p-6 px-12 ml-6 overflow-auto">
...
const login = (): void => {
  ...
  } else if (loginForm.username === 'student' && loginForm.password === '1234') {
    authStore.setUser(
      1, // ID del usuario
      'Pedro', // Nombre de usuario
      'student@example.com', // Correo electrónico
      'Estudiante Registrado', // Nombre completo
      '12345678-9', // RUT
      'student' // Rol
    );
