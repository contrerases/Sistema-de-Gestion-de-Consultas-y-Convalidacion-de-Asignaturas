<template>
  <main>
      ...
      <div class="overflow-x-auto">
        <table class="table-auto w-full bg-card text-card-foreground rounded-xl shadow-md border border-border">
          <thead class="bg-muted text-muted-foreground">
            <tr>
              <th class="px-6 py-3 text-left uppercase text-sm font-semibold">RUT</th>
              <th class="px-6 py-3 text-left uppercase text-sm font-semibold">Nombre</th>
              <th class="px-6 py-3 text-left uppercase text-sm font-semibold">Libre a Convalidar</th>
            </tr>
          </thead>
          <tbody>
            <tr 
            ...
            >
              <td colspan="3" class="px-6 py-3 text-center">No hay inscripciones</td>
            </tr>
            <tr 
            ...
            >
              <td class="px-6 py-3 border-t border-border">{{ inscription.rut_student }}</td>
              <td class="px-6 py-3 border-t border-border">{{ inscription.name_student }}</td>
              <td class="px-6 py-3 border-t border-border">{{ inscription.curriculum_course }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="pt-10 flex flex-col items-center space-y-4">
      ...
      <label class="flex flex-col items-center w-64 px-4 py-6 bg-input text-muted-foreground rounded-lg shadow-md cursor-pointer hover:bg-muted/80 transition">
        <!-- Mostrar SVG y texto solo si no hay archivo seleccionado -->
        <template v-if="!selectedFileName">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mb-2 text-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <input type="file" class="hidden" ref="fileInput" accept=".xlsx" @change="handleFileChange" />
      </label>
...
const processExcel = () => {
  if (fileInput.value && fileInput.value.files.length > 0) {
    ...
    reader.onload = (e) => {
      ...
      const jsonData = XLSX.utils.sheet_to_json(sheet) as Array<{ rut: string; nota: string }>;
